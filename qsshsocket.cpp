...
{
    qDebug() << "Stopping QSSHSocket";
    m_run = false;
    qDebug() << "Waiting for thread";
    //this->wait(500);
    this->quit();
    qDebug() << "Thread waiting successful!";
}
    ...
    while(m_run)
    {
        if (m_session == NULL)
        {
            if (m_host != "")
            {
                ...
                ssh_options_set(m_session, SSH_OPTIONS_USER, m_user.toLatin1().data());
        ...
        else if (!m_loggedIn)
        {
            ...
            if (m_user != "")//In order to connect to BBB!??? && m_password !="")
            {
        ...
        else if (!m_currentOperation.executed)
        {
            ...
            else if (m_currentOperation.type == Push)
            {
                ...
                if (ssh_scp_push_file(scpSession, m_currentOperation.remotePath.toLatin1().data(), buffer.size(), S_IRUSR | S_IWUSR) != SSH_OK)
                {
                    ssh_scp_close(scpSession);
                    ssh_scp_free(scpSession);
